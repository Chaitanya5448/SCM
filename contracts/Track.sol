...
contract Track {
    ...
    uint256 _price) public payable {
        require(msg.value == _price, "Payement amount must match the price.");
    ...
    function startShipment(address _sender, address _receiver, uint256 _index) public{
        ...
        require(shipment.status == ShipmentStatus.PENDING, "Shipment alraedy in transit.");

        shipment.status = ShipmentStatus.IN_TRANSIT;
        tyepShipment.status = ShipmentStatus.IN_TRANSIT;

        emit ShipmentInTransit(_sender, _receiver, shipment.pickupTime,0,0);
    ...
    public {
        ...
        emit ShipmentDelivered(_sender, _receiver, shipment.deliveryTime,0,0);
